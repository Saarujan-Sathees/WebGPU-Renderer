...
class Shape {
    constructor(vertices = [ new util.Vertex() ]) {
...
export class Triangle extends Shape {
    constructor(a = new util.Vertex(), b = new util.Vertex(), c = new util.Vertex()) {
        super([ a, b, c ]);
    }

    toArray(preserve = false) {
        if (!this.vertices[0].normal) {
            let arr = new Float32Array(24);
            for (let i = 0; i < 3; ++i) {
                arr.set(this.vertices[i].toArray(), i * 8);
            }

            if (!preserve) this.vertices = null;
            return arr;
        } else {
            let arr = new Float32Array(36);
            for (let i = 0; i < 3; ++i) {
                arr.set(this.vertices[i].toArray(), i * 12);
            }
...
export class Quadrilateral extends Shape {
    constructor(a = new util.Vertex(), b = new util.Vertex(), c = new util.Vertex(), d = new util.Vertex()) {
        super([a, b, c, d]);
    }

    toArray(preserve = false) {
        let arr = new Float32Array(48);
        arr.set(new Triangle(this.vertices[0], this.vertices[1], this.vertices[2]).toArray(), 0);
        arr.set(new Triangle(this.vertices[1], this.vertices[2], this.vertices[3]).toArray(), 24);
        if (!preserve) this.vertices = null;
...
export class Box extends Shape {
    constructor(min = new util.Vertex(), max = new util.Vertex()) {
        super([min, max]);
        this.origin = min.position.add(max.position).divNum(2);
        this.emission = min.color.add(max.color).divNum(2);
    }

    toArray() {
        let arr = new Float32Array(288);
        const min = this.vertices[0].position, max = this.vertices[1].position;

        let vertices = [
            this.vertices[0],
            new util.Vertex(new util.Vector(min.get(0), min.get(1), max.get(2)), this.emission),
            new util.Vertex(new util.Vector(min.get(0), max.get(1), min.get(2)), this.emission),
            new util.Vertex(new util.Vector(max.get(0), min.get(1), min.get(2)), this.emission),
            this.vertices[1],
            new util.Vertex(new util.Vector(max.get(0), max.get(1), min.get(2)), this.emission),
            new util.Vertex(new util.Vector(max.get(0), min.get(1), max.get(2)), this.emission),
            new util.Vertex(new util.Vector(min.get(0), max.get(1), max.get(2)), this.emission),
        ]

        arr.set(new Quadrilateral(vertices[0], vertices[1], vertices[2], vertices[7]).toArray(), 0); //Left
        arr.set(new Quadrilateral(vertices[3], vertices[6], vertices[5], vertices[4]).toArray(), 48); //Right

        arr.set(new Quadrilateral(vertices[0], vertices[2], vertices[3], vertices[5]).toArray(), 96); //Back
        arr.set(new Quadrilateral(vertices[4], vertices[6], vertices[7], vertices[1]).toArray(), 144); //Front

        arr.set(new Quadrilateral(vertices[0], vertices[1], vertices[3], vertices[6]).toArray(), 192); //Bottom
        arr.set(new Quadrilateral(vertices[4], vertices[5], vertices[7], vertices[2]).toArray(), 240); //Top
...
export class Terrain {
    dimensions = new util.Vector();
    worldDimensions = new util.Vector();
    vertexData = [];
    noiseMap = new util.PerlinNoise();
    coord = [];
    maxHeight = 10;
    halfHeight = 5;
    color = new util.Color(255, 255, 255);
    xOff = new util.Vector(1, 0, 0);
    yOff = new util.Vector(0, 1, 0);
    zOff = new util.Vector(0, 0, 1);
    cornerFromA = [ 0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3 ];
    cornerFromB = [ 1, 2, 3, 0, 5, 6, 7, 4, 4, 5, 6, 7 ];
    lookup = [
        ...
        [ 2, 1, 5, 2, 5, 8, 2, 8, 11, 4, 8, 5 ], [ 10, 3, 11, 10, 1, 3, 9, 5, 4 ], [ 4, 9, 5, 0, 8, 1, 8, 10, 1, 8, 11, 10 ],
    ...
    bindCoords(c = new util.Vector()) {
        if (c.get(0) >= this.dimensions.get(0) || c.get(1) >= this.dimensions.get(1) || c.get(2) >= this.dimensions.get(2)) 
            return this.dimensions.subNum(1);
        else if (c.get(0) < 0 || c.get(1) < 0 || c.get(2) < 0) 
            return new util.Vector(0, 0, 0);
        
        return c;
    }

    density(c = new util.Vector()) {
        c = this.bindCoords(c);
        let density = -c.get(1); 
        
        let warp = this.noiseMap.generateAt(c.mult(new util.Vector(0.004, 0, 0.004)), 1, 1, 1, 1, 1);
        density += this.noiseMap.generateAt(c.addNum(warp * 8).mult(new util.Vector(1, 0, 1)), 4, 0.01, 8, 2.01, 0.51);// + this.halfHeight + this.noiseMap.get(c.get(0), c.get(2));
        return density;
    }

    toWorld(c) {
        return c.div(this.dimensions).mult(this.worldDimensions);
    }

    calcNormal(c) {
        if (c.get(0) >= this.dimensions.get(0) - 1 || c.get(1) >= this.dimensions.get(1) - 1|| c.get(2) >= this.dimensions.get(2) - 1) 
            c = this.dimensions.subNum(1);
        else if (c.get(0) <= 0 || c.get(1) <= 0 || c.get(2) <= 0) 
            c = new util.Vector(1, 1, 1);

        return util.normalize(new util.Vector(this.density(c.sub(this.xOff)) - this.density(c.add(this.xOff)), 
                                              this.density(c.sub(this.yOff)) - this.density(c.add(this.yOff)),
                                              this.density(c.sub(this.zOff)) - this.density(c.add(this.zOff))));
    }

    mix(a, b, t) {
        //return b.sub(a).multNum((3.0 - t * 2.0) * t * t).add(a);
        return a.add(b.sub(a).multNum(t));
    }

    interpolate(a, b) {
        const posA = this.coord[a], posB = this.coord[b];
        const densityA = this.density(posA), t = (-densityA) / (this.density(posB) - densityA);
        const normA = this.calcNormal(posA), normB = this.calcNormal(posB);
    
        return new util.Vertex(this.mix(this.toWorld(posA), this.toWorld(posB), t).mult(new util.Vector(1, 1, -1)), 
                               this.color, null, util.normalize(this.mix(normA, normB, t)));
    }

    constructor(worldDimensions, dimensions, height, cornerPosition = new util.Vector(0, 0, 0)) {
        this.worldDimensions = worldDimensions;
        this.dimensions = dimensions;
        this.maxHeight = height;
        this.halfHeight = dimensions.get(1) * 0.5;
        this.corner = cornerPosition;
    }

    async setNoiseMap(settings) {
        if (!settings.file) {
            this.noiseMap = new util.PerlinNoise(this.dimensions.get(0) + 1, this.dimensions.get(2) + 1);
            /*this.noiseMap.generate(!settings.amplitude ? 100 : settings.amplitude,
                                   !settings.frequency ? 1 : settings.frequency,
                                   !settings.octaves ? 4 : settings.octaves,
                                   !settings.lacunarity ? 1 : settings.lacunarity,
                                   !settings.persistence ? 0.5 : settings.persistence);*/
        } else {
            this.noiseMap = await util.PerlinNoise.fromPNG(`Resources/${settings.file}`);
        }
    }

    generate() {
        let c, index, l, edgeA, edgeB, edgeC;
        for (let x = 0; x < this.dimensions.get(0); ++x) {
            if (x % 50 == 0) console.log(x);
            for (let y = 0; y < this.dimensions.get(1); ++y) {
                for (let z = 0; z < this.dimensions.get(2); ++z) {
                    c = new util.Vector(x, y, z);
                    this.coord = [ c, c.add(new util.Vector(1, 0, 0)), c.add(new util.Vector(1, 0, 1)), 
                                   c.add(new util.Vector(0, 0, 1)), c.add(new util.Vector(0, 1, 0)), 
                                   c.add(new util.Vector(1, 1, 0)), c.addNum(1), c.add(new util.Vector(0, 1, 1)) ];
                    
                    index = 0;
                    for (let i = 0; i < 8; ++i) {
                        if (this.density(this.coord[i]) > 0) {
                            index |= (1 << i);
                        }
                    }
                    ...
                    for (let i = 0; i < l.length; i += 3) {
                        edgeA = l[i], edgeB = l[i + 1], edgeC = l[i + 2];
                        this.vertexData.push(...this.interpolate(this.cornerFromA[edgeA], this.cornerFromB[edgeA]).toArray(),
                                             ...this.interpolate(this.cornerFromA[edgeB], this.cornerFromB[edgeB]).toArray(),
                                             ...this.interpolate(this.cornerFromA[edgeC], this.cornerFromB[edgeC]).toArray());
                    }
                }
            }
        }
        
        this.vertexData.push(0, 0, 0, 1.0, ...this.color.toArr(), 0, 1, 0, 1);
        this.vertexData.push(this.dimensions.get(0), 0, 0, 1.0, ...this.color.toArr(), 0, 1, 0, 1);
        this.vertexData.push(0, 0, -this.dimensions.get(2), 1.0, ...this.color.toArr(), 0, 1, 0, 1);
        this.vertexData.push(this.dimensions.get(0), 0, 0, 1.0, ...this.color.toArr(), 0, 1, 0, 1);
        this.vertexData.push(0, 0, -this.dimensions.get(2), 1.0, ...this.color.toArr(), 0, 1, 0, 1);
        this.vertexData.push(this.dimensions.get(0), 0, -this.dimensions.get(2), 1.0, ...this.color.toArr(), 0, 1, 0, 1);
            
        this.vertexData = Float32Array.from(this.vertexData);
    }

    toArray(preserve = false) {
        let temp = this.vertexData;
